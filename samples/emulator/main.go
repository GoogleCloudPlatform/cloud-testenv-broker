...
var (
  active  = flag.Bool("active", false, "Whether this emulator actively registers with the broker")
...
func newStatusServer(serving bool) *statusServer {
  s := statusServer{serving: serving, mux: http.NewServeMux()}
  s.servingCond = sync.NewCond(&s.mu)
  ...
  s.mux.HandleFunc("/setServing", func(w http.ResponseWriter, r *http.Request) {
    s.mu.Lock()
    s.serving = true
    s.servingCond.Broadcast()
    s.mu.Unlock()
  })
  return &s
}

func (s* statusServer) waitUntilServing() {
  s.servingCond.L.Lock()
  for !s.serving {
    s.servingCond.Wait()
  }
  s.servingCond.L.Unlock()
}
...
func main() {
  ...
  log.Printf("starting status server...")
  done := make(chan bool, 1)
  go func() {
    s.ListenAndServe()
    done <- true
  }()
  statusServer.waitUntilServing()
